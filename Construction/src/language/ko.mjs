export const language = {
    'MODIFIER_DATA_spoof_AddWeaponMasteryMelee': "근접 무기 유형 마스터리 해금",
    'MODIFIER_DATA_spoof_AddWeaponMasteryRanged': "원거리 무기 유형 마스터리 × 해금",
    'MODIFIER_DATA_spoof_AddWeaponMasteryMagic': "마법 무기 유형 마스터리 × 해금",
    'MODIFIER_DATA_spoof_IncreaseMasteryLevelMelee': "근접 무기 유형 최고 마스터리 레벨 증가",
    'MODIFIER_DATA_spoof_IncreaseMasteryLevelRanged': "원거리 무기 유형 최고 마스터리 레벨 증가",
    'MODIFIER_DATA_spoof_IncreaseMasteryLevelMagic': "마법 무기 유형 최고 마스터리 레벨 증가",
    'MODIFIER_DATA_spoof_AddIndBonusMelee': '근접 무기 개별 보너스 해금',
    'MODIFIER_DATA_spoof_AddIndBonusRanged': '원거리 무기 개별 보너스 해금',
    'MODIFIER_DATA_spoof_AddIndBonusMagic': '마법 무기 개별 보너스 해금',
    'MODIFIER_DATA_increaseWeaponXPMelee': "근접 무기 마스터리 경험치 +${value}%",
    'MODIFIER_DATA_increaseWeaponXPRanged': "원거리 무기 마스터리 경험치 +${value}%",
    'MODIFIER_DATA_increaseWeaponXPMagic': "마법 무기 마스터리 경험치 +${value}%",
    'MODIFIER_DATA_spoof_DoubleIndBonusMelee': "근접 무기 개별 보너스 효과 +50%",
    'MODIFIER_DATA_spoof_DoubleIndBonusRanged': "원거리 무기 개별 보너스 효과 +50%",
    'MODIFIER_DATA_spoof_DoubleIndBonusMagic': "마법 무기 개별 보너스 효과 +50%",
    'MODIFIER_DATA_GlobalEffectIgnoreChance': '모든 전투 효과를 무시할 확률 ${value}%',
    'MODIFIER_DATA_itemProtectionChance': '사망 시 아이템을 잃지 않을 확률 ${value}%',
    'MENU_TOOLTIP_TRIPLING': '아이템이 배증되는 대신 ${chance}의 확률로 3배 증가',
    'MODIFIER_DATA_xpperbankslots': '사용 중인 은행 슬롯 100칸당 비전투 스킬 경험치 ${value}%',
    'MODIFIER_DATA_preservationbankslots': '사용 중인 은행 슬롯 50칸당 글로벌 아이템 배증 확률 ${value}%',
    'MODIFIER_DATA_globalGPBanksSlots': '사용 중인 은행 슬롯 50칸당 ${currencyName} 획득량 ${value}% (아이템 판매 제외)',
    'MODIFIER_DATA_skillPetChanceBankSlots': '사용 중인 은행 슬롯 50칸당 스킬 펫 발견 확률 ${value}%',
    'MODIFIER_DATA_expandBankSlotCap1': '사용 중인 은행 슬롯 보너스 적용 범위가 최대 1000칸으로 확장',
    'MODIFIER_DATA_expandBankSlotCap2': '사용 중인 은행 슬롯 보너스 적용 범위 제한 제거',
    'MODIFIER_DATA_unlockTripling': '아이템 배증 확률 제한 해제 및 100%를 초과하는 배증 확률을 3배 증가 확률로 전환',
    'MODIFIER_DATA_spoof_addBankTabs': '은행 탭 ${value}',
'MODIFIER_DATA_blockTheUnblockable': '적의 빗맞힘 확률의 ${value}%를 회피 불가 공격 무효화 확률로 적용',
    'SPLASH_NEGATED': '무효화됨!',
    'MODIFIER_DATA_spoof_bankslotexpl': '사용 중인 은행 슬롯 50칸마다 능력치가 상승하는 모디파이어 획득 (최대 250칸)',

    'MODIFIER_DATA_spoof_expTime': '같은 스킬을 연속으로 수행한 시간당 능력치가 상승하는 모디파이어 획득 (최대 2시간)',
    'MODIFIER_DATA_xpPerTime': '같은 스킬을 연속으로 수행한 시간당 비전투 스킬 경험치 ${value}%',
    'MODIFIER_DATA_preservePerTime': '같은 스킬을 연속으로 수행한 시간당 자원 보존 확률 ${value}%',
    'MODIFIER_DATA_preserveConsPerTime': '소환, 포션 및 소모품 사용 횟수 보존 확률 ${value}%',
    'MODIFIER_DATA_preserveCapPerTime': '같은 스킬을 연속으로 수행한 시간당 자원 보존 확률 상한 ${value}%',
    'MODIFIER_DATA_extendLongBuffs': '같은 스킬을 연속으로 수행할 때 적용되는 보너스 최대 누적 시간 ${value}',
    "ITEM_NAME_Builder_Pouch": "건축가의 주머니",
    "ITEM_NAME_Builder_Pouch_II": "건축가의 주머니 II",
    "ITEM_NAME_Bronze_Crossbow_T_R": "(R) 청동 크로스보우",
    "ITEM_NAME_Iron_Crossbow_T_R": "(R) 철 크로스보우",
    "ITEM_NAME_Steel_Crossbow_T_R": "(R) 강철 크로스보우",
    "ITEM_NAME_Mithril_Crossbow_T_R": "(R) 미스릴 크로스보우",
    "ITEM_NAME_Adamant_Crossbow_T_R": "(R) 애더먼트 크로스보우",
    "ITEM_NAME_Rune_Crossbow_T_R": "(R) 룬 크로스보우",
    "ITEM_NAME_Dragon_Crossbow_T_R": "(R) 드래곤 크로스보우",
    "ITEM_NAME_Green_Dhide_Shield_R": "(R) 녹색 용가죽 방패",
    "ITEM_NAME_Blue_Dhide_Shield_R": "(R) 청색 용가죽 방패",
    "ITEM_NAME_Red_Dhide_Shield_R": "(R) 적색 용가죽 방패",
    "ITEM_NAME_Black_Dhide_Shield_R": "(R) 흑색 용가죽 방패",
    "ITEM_NAME_Ancient_Dhide_Shield_R": "(R) 고대 용가죽 방패",
    "MODIFIER_DATA_decrease_skillEfficiencyChance": "추가 동작 완료 확률 -${value}%",
    "MODIFIER_DATA_increase_skillEfficiencyChance": "추가 동작 완료 확률 +${value}%",
    "MODIFIER_DATA_skillEfficiencyChancePerHamrielStar": "점성술의 헤이미르 성좌에서 최대 레벨에 도달한 별 하나당 추가 동작 완료 확률 ${value}%",
    "MODIFIER_DATA_decrease_skillEfficiencyPotencyForCategory": "${categoryName} 제작 시 추가 동작 완료를 통해 얻는 추가 재료 또는 진행도 -${value}",
    "MODIFIER_DATA_increase_skillEfficiencyPotencyForCategory": "${categoryName} 제작 시 추가 동작 완료를 통해 얻는 추가 재료 또는 진행도 +${value}",
    "MODIFIER_DATA_bypassEfficiencyChance": "추가 동작 완료 확률 ${value}% (게임 모드 제한 무시)",
    "MODIFIER_DATA_unlockEfficiencySkill": "${skillName}에서 추가 동작을 수행할 수 있는 능력 잠금 해제",
    "MODIFIER_DATA_spoofUpgradeCross": "화살 제작에서 만든 크로스보우를 강화할 수 있는 능력 잠금 해제",
    "MODIFIER_DATA_spoofUpgradeShields": "공예에서 만든 방패를 강화할 수 있는 능력 잠금 해제",
    "MODIFIER_DATA_2turnstunoncritchance": "치명타 적중 시 ${value}% 확률로 적을 2턴 동안 기절시킴",
    "MODIFIER_DATA_skillScopedPoolCap": "${skillName}의 마스터리 풀 XP 상한 ${value}%",
    "MODIFIER_DATA_spoofAddNewMPBonus": "${skillName}의 마스터리 풀 체크포인트 ${value}",
    "MODIFIER_DATA_spoofAddConsToSmithingPool": "대장기술의 마스터리 풀 체크포인트에 건설 보너스 추가",
    "MODIFIER_DATA_findChuckOnFixtureComp": "가구 완성 시 낮은 확률로 척 발견",
    "MODIFIER_DATA_maxlevelAssocbonusEff": "가구 제작 시 관련 스킬 레벨이 99 이상인 경우 추가 동작 완료 확률 ${value}%",
    'MODIFIER_DATA_skillItemDoublingChanceGlobal': '스킬에서 아이템이 2배가 될 확률 ${value}%',
    'AGI_OBSTACLE_WarmUpStretches': "워밍업 스트레칭",
    'AGI_OBSTACLE_AltarHop': "제단 점프",
    'AGI_OBSTACLE_WatchtowerClimb': "감시탑 등반",
    'AGI_OBSTACLE_MidasTrap': "미다스 함정",
    'AGI_OBSTACLE_VictoryHouseLap': "승리의 집 한 바퀴",

    'SPECIAL_ATTACK_DESC_MAGIC_WEAPON': '위력이 낮은 공격 3회. 각 공격은 낮은 확률로 화상, 서리화상, 독을 부여. 고유 효과.',
    'SPECIAL_ATTACK_DESC_MELEE_WEAPON': '위력이 크게 증가한 공격. 고유 효과.',
    'SPECIAL_ATTACK_DESC_RANGED_WEAPON': '위력이 약간 낮은 공격 2회. 이 공격은 절대 빗나가지 않음. 고유 효과.',
    'SPECIAL_ATTACK_NAME_Mana_Surge': '마나 서지',
    'SPECIAL_ATTACK_NAME_Twin_Shot': '트윈 샷',
    'SPECIAL_ATTACK_NAME_Brutal_Strike': '브루탈 스트라이크',
    'MODIFIER_DATA_spoof_AddMeleeSpecialAttack': '모든 근접 무기에 "브루탈 스트라이크" 특수 공격 추가',
    'MODIFIER_DATA_spoof_AddRangedSpecialAttack': '모든 원거리 무기에 "트윈 샷" 특수 공격 추가',
    'MODIFIER_DATA_spoof_AddAgiObstacle': '${skillName}에서 새로운 Tier ${value} 장애물 사용 가능',
    'MODIFIER_DATA_xpPer5Fixture': 'Tier 5+ 가구 1개당 모든 스킬 ${value}% 경험치',
    'MODIFIER_DATA_hpPerAgiLevels': '민첩 20레벨마다 ${value} 숨겨진 방어 및 HP 레벨',
    'MODIFIER_DATA_IntPerAgiLevels': '민첩 20레벨마다 ${value}% 도둑질 및 민첩 간격',
    'MODIFIER_DATA_CombIntPerAgiLevels': '민첩 20레벨마다 ${value}초 공격 및 몬스터 재생 간격',
    'MODIFIER_DATA_fuckYourGold': '${currencyName} 획득 시, 대신 ${value}% 손실',
    'MODIFIER_DATA_spoof_Damocles': '이 비용을 감당할 수 없으면 사망',
    'MAP_POI_House': "항구 작업실",
    'MAP_POI_House_Desc': '늪의 지독한 안개가 걷히자 귀에는 못을 두드리는 소리와 나무를 켜는 톱질 소리가 들려온다. 분주한 두 부두 사이에는 수십 명의 일꾼들이 크고 작은 온갖 작업을 수행하는 다층 건물이 펼쳐져 있다. 작은 선반부터 교회 창문, 왕의 방으로 보내질 금속 왕좌까지 다양한 물건이 만들어진다. 가구를 싣고 떠나는 배가 하나 있을 때마다, 주문과 자재를 가득 실은 배 두 척이 더 정박한다.',
    'MAP_POI_Yard': "고블린 작업장",
    'MAP_POI_Yard_Desc': '갈라진 틈과 낭떠러지가 가득한 절벽 위에서 분주한 작업장을 발견한다. 온갖 모양과 색의 고블린들이 귀가 찢어질 듯한 기계를 사용해 엄청난 양의 목재, 돌, 금속, 가죽을 가공 가능한 형태로 잘라낸다. 기계는 자주 고장 나고, 고블린들은 그것을 고치기 위해 더 큰 기계를 사용한다. 그 더 큰 기계는 항상 고장 나 있다.',
    'ITEM_NAME_Pile_Of_Bricks': '벽돌 더미',
    'MODIFIER_DATA_bypassNonCombatSkillLevelAR': '모든 비전투 기술 레벨을 ${value}만큼 증가',
    'TOWNSHIP_TASKS_REQ': '${qty1} / ${qty2} ${fixImg}${fixName} 제작',
    'SHOP_NAME_Hidden_Routes': "비밀 경로",
    'ITEM_NAME_Star_Necklace': '별 목걸이',
    'ITEM_NAME_Ice_Gloves': '얼음 장갑',
    'MODIFIER_DATA_ReduceThievingTargetMaxHitBasedOnDefLevel': '방어 레벨마다 도둑 NPC의 최대 피해 감소 (최대 절반까지)',
    'MODIFIER_DATA_spoof_LowerTraderRequirements': '타운십 상인의 레벨 및 임무 요구 사항을 ${value}만큼 감소',
    'MODIFIER_DATA_UnlockTrader': '타운십 상인 잠금 해제',
    'MODIFIER_DATA_spoof_AddItemsToShop': '상점에 새로운 아이템 추가',
    'MODIFIER_DATA_spoof_AddBonusToMiningMasteryPool1': '광산의 10% 및 25% 숙련치 풀 보너스에 대장장이 보너스 추가',
    'MODIFIER_DATA_spoof_AddBonusToMiningMasteryPool2': '광산의 50% 및 95% 숙련치 풀 보너스에 대장장이 보너스 추가',
    'MODIFIER_DATA_spoof_upgradeSlayerShtuff': '슬레이어 지역 접근을 제공하는 아이템 강화',
    'MODIFIER_DATA_doublepetsmillion': '슬레이어 코인이 100만 이상이면 레벨 100까지 전투 지역에서 펫 발견 확률 2배',
    'MODIFIER_DATA_spoof_AddSlayerEnemies1': '묘지에서 숨겨진 적 발견',
    'MODIFIER_DATA_spoof_AddSlayerEnemies2': '산적 소굴에서 숨겨진 적 발견',
    'MODIFIER_DATA_spoof_AddSlayerEnemies3': '얼음 언덕에서 숨겨진 적 발견',
    'MODIFIER_DATA_spoof_AddSlayerEnemies4': '마법사 탑에서 숨겨진 적 발견',
    'MODIFIER_DATA_spoof_AddSlayerEnemies5': '드래곤 계곡에서 숨겨진 적 발견',
    'MODIFIER_DATA_slayerCostReduction': '슬레이어 임무 비용 ${value}% 감소',
    'ITEM_NAME_Engraved_Diagram_Fragment': '조각된 도면 조각',
    'ITEM_NAME_Engraved_Diagram': '조각된 제작 도면',
    'TOASTS_FIXTURE_TIER_REQUIRED': '이 작업을 수행하려면 Tier ${tier} ${fixtureName}가 필요합니다!',
    'MENU_TEXT_REQUIRES_FIXTURE_TIER': '${fixtureImage}${fixtureName}를 Tier ${tier}까지 제작',
    'ITEM_NAME_Half_Burnt_Ledger': '반쯤 탄 장부',

    'ITEM_NAME_Engraved_Construct_Diagram': '조각된 제작 도면',
    'RIELK_CONMOD_SLAYER2': '슬레이어 임무 몬스터와 싸울 때 모든 공격 피해 +7%',
    'RIELK_CONMOD_SLAYER5': '슬레이어 임무 몬스터와 싸울 때 소모 슬롯 충전 유지 확률 +15%',
    'MENU_TEXT_ENGRAVED_DIAGRAM_TEXT_0': '제작물 만드는 방법:',
    'MENU_TEXT_ENGRAVED_DIAGRAM_TEXT_1': '1. 강철처럼 단단하게',
    'ENEMY_NAME_HauntedCoffin': '유령 들린 관',
    'ENEMY_NAME_SmugglerCarriage': '밀수꾼 마차',
    'ENEMY_NAME_IceSculpture': '얼음 조각상',
    'MENU_TEXT_ENGRAVED_DIAGRAM_TEXT_2': '2. 항상 자신을 믿어라',
    'MENU_TEXT_ENGRAVED_DIAGRAM_TEXT_3': '다른 단계는 읽을 수 없지만, 아마 이와 비슷할 것이다',
    'MENU_TEXT_ENGRAVED_DIAGRAM_UNLOCK_0': '받는 피해를 영구적으로 2% 감소',
    'MENU_TEXT_ENGRAVED_DIAGRAM_UNLOCK_1': '최대 피해의 2%를 최소 피해에 영구적으로 추가',
    'MODIFIER_DATA_Spoof_UnlockMarkSuperLevels': '${skillName}에서 마크의 최대 레벨이 ${value}만큼 증가합니다',
    'MODIFIER_DATA_Spoof_UnlockMarkSuperLevels': '${skillName}에서 마크의 최대 레벨이 ${value}만큼 증가합니다',
    'MODIFIER_DATA_TabletFightBuffBasedOnMarkLevel': '마크 레벨 5 초과 1레벨당 소환수 최대 피해량 ${value}% 증가',
    'MODIFIER_DATA_TabletAmountBuffBasedOnMarkLevel': '마크 레벨 5 초과 1레벨당 ${skillName}에서 획득하는 태블릿 기본 수량 ${value} 증가',
    'MODIFIER_DATA_TabletEffectBuffBasedOnMarkLevel': '마크 레벨 5 초과 1레벨당 소환수 보너스 ${value}% 증가',
    'MODIFIER_DATA_IncreaseMarkChance': '장착한 상태에서 해당 스킬을 훈련할 때 마크 드롭률이 2배로 증가',
    'SKILL_RELIC': '${skill} 유물 ${number}',
    'SKILL_MASTER_RELIC': '${skill} 마스터 유물',
    'MENU_TEXT_MARKS_UPGRADED': '이 소환수를 장착한 상태에서 해당 스킬을 훈련하면, 해당 마크의 획득률이 <strong class="construction-success fuck-you">4배</strong>가 됩니다.',
    'MENU_TEXT_SUMMONING_MARK_POPUP': '이 소환수와의 <span class="font-w700 construction-success">유대가 깊어졌습니다</span>!<br>당신의 ${spiritimg}<span class="font-w700 construction-success">${spiritname}</span> 등급에 따라 달라집니다.',
    'ITEM_NAME_Summoning_Familiar_Bee': '벌',
    'MODIFIER_DATA_skillEfficiencyChanceforSubcategory': '${skillName}에서 ${subcategoryName}을 제작할 때 ${value}% 확률로 추가 행동을 완료합니다',
    'MODIFIER_DATA_nullifyPrayerPointsUnder': '총합이 ${value} 이하인 활성화된 기도 포인트(감소 전)는 무료가 되지만, ${skillName} 스킬 XP는 획득하지 않습니다',
    'MODIFIER_DATA_skillItemDoublingChanceSubcategory': '${skillName}에서 ${subcategoryName}을 제작할 때 ${value}% 확률로 아이템을 두 배로 획득합니다',
    'RIELK_SYNERGY_Pig_Bee': '패시브 요리가 활성화되지 않은 동안, 두 배 획득 및 보존 확률 +10%', 'RIELK_SYNERGY_Bear_Bee': '건설에서 “두 번 측정” 물약이 활성화된 동안, 추가 행동 완료 시 자원 비용이 0.2 감소',
    'CONSTRUCTION_FIXTURE_NAME_ Spirit_Catcher': '영혼 포획기',
    'MODIFIER_DATA_allCombatSkillsFlatHiddenLevel': '모든 전투 스킬에 ${value} 숨겨진 레벨',
    'MODIFIER_DATA_allNonCombatSkillsFlatHiddenLevel': '모든 비전투 스킬에 ${value} 숨겨진 레벨',
    'MODIFIER_DATA_bypassNonCombatSkillLevelCapAR': '모든 비전투 기술의 현재 최대 레벨을 ${value}만큼 증가시킵니다 (최대 한도까지)',
    'MODIFIER_DATA_bypassGlobalDoubling': '전역적으로 아이템을 ${value}% 확률로 두 배 획득 (게임 모드 제한 무시)',
    'ITEM_NAME_Construction_Lesser_Relic': '건설 하급 유물',

    'COMBAT_MISC_SHIELD_BROKEN': '워드',
    'MENU_TOOLTIP_CONVERGENCE': '컨버전스',
    'MENU_TOOLTIP_GOLDVERGENCE': '황금 컨버전스',
    'MENU_TOOLTIP_CONVERGENCE_EXPL': '별을 연구하는 동안 ${rune}과 ${dust}를 함께 사용하면 ${conv}가 발생합니다.',
    'MENU_TOOLTIP_CONVERGENCE_CHANCE': '${conv} 발생 확률:',
    'MENU_TOOLTIP_CONVERGENCE_LOCKED': '이 별자리를 더 탐험하여 이 별을 정렬하세요',
    'SPECIAL_ATTACK_DESC_MAGIC_WEAPON': '약화된 힘으로 세 번 공격합니다. 각 공격은 낮은 확률로 화상, 냉화상 또는 독을 부여합니다',
    'MODIFIER_DATA_runeShieldMultiplier': '워드를 생성할 때 필요한 룬을 절반으로 감소',
    'MODIFIER_DATA_addRuneShield': '전투에서 300 룬을 사용하면 적의 다음 턴을 무효화하는 워드를 생성합니다',
    'MODIFIER_DATA_fake_Book_of_Eli': 'Tier III 오로라는 장착된 책에 관계없이 사용할 수 있습니다',
    'MODIFIER_DATA_spoof_AddMagicSpecialAttack': '모든 마법 무기에 "마나 급증" 특수 공격을 추가합니다',
    'MODIFIER_DATA_AllowAurorasAnytime': '마법 무기 없이 오로라를 사용할 수 있으며, 룬 비용은 3배입니다',
    'MODIFIER_DATA_spoof_AddRunesAstrology': '점성술을 연구하는 동안 별자리에서 대량의 룬 에센스를 드물게 얻습니다',
    'MODIFIER_DATA_ExtraTier1Potions': '레벨 99 숙련으로 포션을 만들 때 ${value}개의 추가 Tier I 포션 획득',
    'MODIFIER_DATA_reducePotionUpReq': '은행에서 포션을 강화하려면 ${value} 숙련 레벨 필요',
    'MODIFIER_DATA_ChangeAddiIntoHighTier': 'Herblore에서 얻는 무작위 포션은 현재 포션 등급보다 낮을 수 없습니다',
    'MODIFIER_DATA_spoofReduceUpAmount': '은행에서 포션을 강화하려면 ${value}개의 포션 필요',
    'MODIFIER_DATA_spoofIncreaseRuneReduction': '마법 무기의 룬 비용 ${value} 감소',
    'MODIFIER_DATA_runecraftingRuneCostReduction': '-${skillName}에서 장비 또는 무기의 룬 비용 ${value}% 감소 (1 이하로는 감소 불가)',
    'MODIFIER_DATA_ComboRunesonElemRunes': '원소 룬 제작 시 ${skillName}에서 ${value} 관련 조합 룬 획득',
    'MODIFIER_DATA_runePreservationCap': '룬 보존 한도: ${value}%',
    'MODIFIER_DATA_spoof_UpgradeScrollOfEssence': '${skillName}에서 룬 에센스를 찾을 확률을 높이기 위해 "에센스 두루마리" 강화',
    'MODIFIER_DATA_spoof_AddStarStandardLevel': '표준 별 최대 레벨 ${value} 증가',
    'MODIFIER_DATA_UnlockConvergence': '컨버전스 잠금 해제',
    'ENEMY_NAME_Basic_Golem': '아케인 골렘',
    'ENEMY_NAME_Advanced_Golem': '고급 아케인 골렘',
    'ENEMY_NAME_Expert_Golem': '전문 아케인 골렘',

    'SKILL_NAME_Construction': '구조',

    'GUIDE_CONSTR_HERO': '${ConstructionIcon64}${constructionTitle}${ModioLink}',
    'GUIDE_CONSTR_SUB1': '여러 기술에서 모은 재료, 유물, 기념품을 사용해 ${HouseIcon24}${HouseWordStyled}를 발전시키세요.',
    'GUIDE_CONSTR_SUB2': '${ConstructionIcon24}${constructionTitle}은(는) 게임 진행에 따라 점점 더 큰 보너스를 제공합니다.',

    'GUIDE_SECTION_MATERIALS_TITLE': '필요한 재료',
    'GUIDE_SECTION_MECH_TITLE': '추가 메커니즘',
    'GUIDE_SECTION_HOUSE_TITLE': '내 집',
    'GUIDE_SECTION_EXTENDED_TITLE': '고급 기능',

    'GUIDE_PLANKS_NAME': '판재',
    'GUIDE_NAILS_NAME': '못',
    'GUIDE_STRAPS_NAME': '끈',
    'GUIDE_BRICKS_NAME': '벽돌',

    'GUIDE_PLANKS_TEXT': '${Beam16}${Beams}로 만들며, ${Fletching16}${Fletching}에서 가공합니다. 원목은 ${Woodcutting16}${Woodcutting}로 얻습니다.',
    'GUIDE_NAILS_TEXT': '${Bar16}${Bars}로 만들며, ${Smithing16}${Smithing}에서 제련합니다. 광석은 ${Mining16}${Mining}에서 채굴합니다.',
    'GUIDE_STRAPS_TEXT': '${Slivers16}${Slivers}로 만들며, ${Crafting16}${Crafting}에서 제작합니다. 가죽은 ${Shop16}${Shop}에서 구입합니다.',
    'GUIDE_BRICKS_TEXT': '${Rune16}${Runes}로 만들며, ${Runecrafting16}${Runecrafting}에서 제작합니다. 룬 정수는 ${Mining16}${Mining}에서 얻습니다.',

    'GUIDE_EFFICIENCY_TITLE': '효율',
    'GUIDE_EFFICIENCY_P1A': '${constructionTitle}에서만 사용 가능한 ${Efficiency16}효율은 행동의 비용과 산출량을 곱해 증가시킵니다.',
    'GUIDE_EFFICIENCY_P1B': '${BothStrong} 활성화 시 획득되는 재료의 확률과 수량을 모두 증가시킬 수 있습니다.',
    'GUIDE_EFFICIENCY_P2': '${MaterialsCat16}${materialsWord} 제작 시 효율은 결과에 곱연산으로 적용됩니다.${BR}${FurnitureCat16}${furnitureWord} 제작 시에는 소수점 보너스가 무시됩니다.',
    'GUIDE_EFFICIENCY_WARNING': '효율의 재료/진행 보너스를 증가시켜도 행동 비용은 증가하지 않습니다.',

    'GUIDE_HOUSE_P1': '${FurnitureCat16}${furnitureWord}를 건설하고 업그레이드하면 ${HouseIcon16}${houseWord}의 등급이 올라가며, ${constructionTitle}에 큰 보너스를 제공합니다.',
    'GUIDE_HOUSE_P2': '내 ${houseWord}의 모든 ${FurnitureCat16}${furnitureWord}가 특정 등급에 도달하면 ${houseWord}의 등급이 상승합니다.',
    'GUIDE_HOUSE_P3': '${houseWord}를 완성하면 게임 전반에 걸쳐 큰 보상을 제공하며, 이는 ${ConstructionIcon16}${constructionTitle}의 궁극적인 목표입니다.',

    'GUIDE_EXTENDED_TITLE': '추가 콘텐츠',
    'GUIDE_EXTENDED_P1': 'Construction 모드는 게임 전반에 새로운 적, 아이템, 레시피, 교환을 추가하여 세계와의 통합을 더욱 깊게 합니다.',
    'GUIDE_EXTENDED_P2': '모든 추가 사항의 전체 목록은 ${ChangeListLink}에서 확인할 수 있습니다. (${ExternalLinkNote})',

    'GUIDE_TERM_CONSTRUCTION': '건설',
    'GUIDE_TERM_FURNITURE': '가구',
    'GUIDE_TERM_MATERIALS': '재료',
    'GUIDE_TERM_BEAMS': '목재 보',
    'GUIDE_TERM_BARS': '주괴',
    'GUIDE_TERM_SLIVERS': '조각',
    'GUIDE_TERM_RUNES': '룬',
    'GUIDE_TERM_FLETCHING': '화살 제작',
    'GUIDE_TERM_WOODCUTTING': '벌목',
    'GUIDE_TERM_SMITHING': '대장기술',
    'GUIDE_TERM_MINING': '채굴',
    'GUIDE_TERM_CRAFTING': '제작',
    'GUIDE_TERM_SHOP': '상점',
    'GUIDE_TERM_BOTH': '둘 다',

    'THIEVING_NPC_BRICKLAYER': '벽돌공',
    'THIEVING_NPC_CONSTRUCT': '콘스트럭트',
    'ITEM_NAME_Steel_Toed_Boots': '강철 앞코 부츠',
    'ITEM_NAME_Mystic_Rotating_Saw': '신비한 회전 톱',

    'GUIDE_LINK_HERE': '여기',
    'GUIDE_EXTERNAL_LINK_NOTE': '외부 링크(영어)',
    'GUIDE_TOOLTIP_NEW': '건설이 처음이신가요?<br/> <span class="construction-victory">게임 가이드</span>에서 시작해 보세요!',
    'GUIDE_TOOLTIP_UPDATE': '건설이 <span class="text-warning">대형 업데이트</span>를 받았습니다! <br/> 자세히 알아보려면 클릭!',

    'MENU_TEXT_SELECT_CONSTRUCTION_CATEGORY': '건설 카테고리를 선택하세요',
    'MENU_TEXT_CONSTRUCT': '짓다',
    'MENU_TEXT_BUILD': '짓다',
    'MENU_TEXT_BUILT_PROGRESS': '증가: ${currentValue}/${maxValue}',
    'MENU_TEXT_PARTIAL_BUILT_PROGRESS': '${currentValue}/${maxValue} ${percent}',
    'MENU_TEXT_SHOW_UPGRADES': '업그레이드',
    'MENU_TEXT_SHOW_GO_BACK': '반품',
    'MENU_TEXT_REMAINING': '남은 비용',
    'MENU_TEXT_SHOW_ALL_ACTIVE_MODIFIERS': '모든 활성 수정자 보기',
    'MENU_TEXT_ALL_ACTIVE_CONSTRUCTION_MODIFIERS': '모든 활성 건설 수정자',
    'MENU_VIEW_HOUSE_TIERS': '하우스 단계 보너스 보기',
    'MENU_UNLOCKED_MASTERY_FOR_TIER': '당신의 하우스 단계가 <span class="construction-victory">단계 ${tiername}</span>로 올랐습니다',
    'MENU_HOUSE_TIER_BONUS_UNLOCKED': '하우스 단계 보너스가 해제되었습니다!',
    'MENU_HOUSE_COMPLETION': '하우스 완성도',
    'MENU_BUILT': '완성됨',
    'MENU_FIXTURES': '설치물',
    'MENU_MAX_TIER': '최대 티어',

    'MENU_TIER': '등급 ${tiername}',
    'MENU_HOUSE': "집",

    'MENU_TEXT_WHAT_ARE_HOUSE_TIERS': '하우스 단계란 무엇인가요?',
    'MENU_TEXT_HOUSE_TIER_EXPLANATION_1': '당신의 하우스 단계는 0에서 시작합니다. 가구를 만들고 각 방을 채우면 하우스 단계가 향상됩니다.</span>',
    'MENU_TEXT_HOUSE_TIER_EXPLANATION_2': '모든 방에 필요한 등급 이상의 가구가 배치되면, 집 등급이 상승합니다. 영구적인 건축 보너스를 잠금 해제하고 자원을 획득합니다.',
    'MENU_TEXT_REWARDS': '보상',
    'MENU_TEXT_TOOLTIP_EFFICIENCY_ARTISAN': '한 번에 ${potency}개의 작업을 완료할 수 있습니다.',
    'MENU_TEXT_TOOLTIP_EFFICIENCY_BUILD_1': '한 번에 1단계 진행을 완료할 수 있습니다.',
    'MENU_TEXT_TOOLTIP_EFFICIENCY_BUILD_2': '한 번에 2단계 진행을 완료할 수 있습니다.',
    'MENU_TEXT_TOOLTIP_EFFICIENCY_BUILD_3': '한 번에 3단계 진행을 완료할 수 있습니다.',
    'MENU_TEXT_TOOLTIP_EFFICIENCY_BUILD_4': '한 번에 4단계 진행을 완료할 수 있습니다.',
    'MENU_TEXT_TOOLTIP_EFFICIENCY_BUILD_5': '한 번에 5단계 진행을 완료할 수 있습니다.',
    'MENU_TEXT_TOOLTIP_EFFICIENCY_BUILD_6': '한 번에 6단계 진행을 완료할 수 있습니다.',
    'MENU_TEXT_TOOLTIP_EFFICIENCY_BUILD_7': '한 번에 7단계 진행을 완료할 수 있습니다.',
    'MENU_TEXT_TOOLTIP_EFFICIENCY_BUILD_8': '한 번에 8단계 진행을 완료할 수 있습니다.',
    'MENU_TEXT_TOOLTIP_EFFICIENCY_BUILD_9': '한 번에 9단계 진행을 완료할 수 있습니다.',
    'MENU_TEXT_TOOLTIP_EFFICIENCY_BUILD_10+': '한 번에 ${potency}단계 진행을 완료할 수 있습니다.',
    'MENU_TOOLTIP_EFFICIENCY_COST': '자원을 ${cost}배 소비합니다.',
    'MENU_EFFICIENCY_BASE': '기본',
    'SHOP_NAME_Construction_Skillcape': '건설 스킬케이프',
    'MISC_TEXT_Saw': '청동 톱',
    'MENU_TEXT_Saw': '현재 톱',
    'SHOP_NAME_Iron_Saw': '철 톱',
    'SHOP_NAME_Steel_Saw': '강철 톱',
    'SHOP_NAME_Black_Saw': '검은 톱',
    'SHOP_NAME_Mithril_Saw': '미스릴 톱',
    'SHOP_NAME_Adamant_Saw': '아다만트 톱',
    'SHOP_NAME_Rune_Saw': '룬 톱',
    'SHOP_NAME_Dragon_Saw': '드래곤 톱',

    'SHOP_DESCRIPTION_Iron_Saw': '건설 간격 -5% (톱 효과 총합: -5%). 추가 행동 완료 시 재료 또는 진행도 +0.20 (총합 +0.20, 톱 효과).',
    'SHOP_DESCRIPTION_Steel_Saw': '건설 간격 -5% (톱 효과 총합: -10%). 추가 행동 완료 시 재료 또는 진행도 +0.20 (총합 +0.40, 톱 효과).',
    'SHOP_DESCRIPTION_Black_Saw': '건설 간격 -5% (톱 효과 총합: -15%). 추가 행동 완료 시 재료 또는 진행도 +0.20 (총합 +0.60, 톱 효과).',
    'SHOP_DESCRIPTION_Mithril_Saw': '건설 간격 -5% (톱 효과 총합: -20%). 추가 행동 완료 시 재료 또는 진행도 +0.20 (총합 +0.80, 톱 효과).',
    'SHOP_DESCRIPTION_Adamant_Saw': '건설 간격 -5% (톱 효과 총합: -25%). 추가 행동 완료 시 재료 또는 진행도 +0.20 (총합 +1.00, 톱 효과).',
    'SHOP_DESCRIPTION_Rune_Saw': '건설 간격 -5% (톱 효과 총합: -30%). 추가 행동 완료 시 재료 또는 진행도 +0.20 (총합 +1.20, 톱 효과).',
    'SHOP_DESCRIPTION_Dragon_Saw': '건설 간격 -10% (톱 효과 총합: -40%). 추가 행동 완료 시 재료 또는 진행도 +0.30 (총합 +1.50, 톱 효과).',

    'PET_NAME_ChuckTheForeman': '포어맨 척',
    'PET_NAME_Scoobs': '스쿱스',
    'PET_DESCRIPTION_Scoobs': "집 5단계 도달",


    'ITEM_NAME_Construction_Skillcape': '건설 스킬케이프',
    'ITEM_NAME_Builders_Hat': '빌더 모자',
    'ITEM_NAME_Builders_Body': '빌더스 바디',
    'ITEM_NAME_Builders_Leggings': '빌더 레깅스',
    'ITEM_NAME_Builders_Boots': '빌더 부츠',

    'ITEM_NAME_Bronze_Nails': '청동 못',
    'ITEM_NAME_Iron_Nails': '철못',
    'ITEM_NAME_Steel_Nails': '강철 못',
    'ITEM_NAME_Mithril_Nails': '미스릴 손톱',
    'ITEM_NAME_Adamantite_Nails': '아다만타이트 손톱',
    'ITEM_NAME_Dragonite_Nails': '드래고나이트 네일',

    'ITEM_NAME_Normal_Beams': '일반 들보',
    'ITEM_NAME_Oak_Beams': '참나무 들보',
    'ITEM_NAME_Teak_Beams': '티크 들보',
    'ITEM_NAME_Mahogany_Beams': '마호가니 들보',
    'ITEM_NAME_Redwood_Beams': '레드우드 들보',

    'ITEM_NAME_Normal_Planks': '일반 판자',
    'ITEM_NAME_Oak_Planks': '참나무 판자',
    'ITEM_NAME_Teak_Planks': '티크 판자',
    'ITEM_NAME_Mahogany_Planks': '마호가니 판자',
    'ITEM_NAME_Magic_Planks': '마법의 판자',
    'ITEM_NAME_Redwood_Planks': '레드우드 판자',
    'ITEM_DESCRIPTION_Magic_Planks': '더 이상 사용되지 않습니다. 레드우드 판자로 교환할 수 있습니다.',

    'ITEM_NAME_Green_Dhide_Leather_Straps': '초록 용가죽 끈',
    'ITEM_NAME_Blue_Dhide_Leather_Straps': '파란 용가죽 끈',
    'ITEM_NAME_Red_Dhide_Leather_Straps': '빨간 용가죽 끈',
    'ITEM_NAME_Black_Dhide_Leather_Straps': '검은 용가죽 끈',
    'ITEM_NAME_Elder_Dhide_Leather_Straps': '엘더 용가죽 끈',

    'ITEM_NAME_Green_Dhide_Leather_Slivers': '초록 용가죽 조각',
    'ITEM_NAME_Blue_Dhide_Leather_Slivers': '파란 용가죽 조각',
    'ITEM_NAME_Red_Dhide_Leather_Slivers': '빨간 용가죽 조각',
    'ITEM_NAME_Black_Dhide_Leather_Slivers': '검은 용가죽 조각',
    'ITEM_NAME_Elder_Dhide_Leather_Slivers': '엘더 용가죽 조각',


    'ITEM_NAME_Clay_Bricks': '점토 벽돌',
    'ITEM_NAME_Limestone_Bricks': '석회암 벽돌',
    'ITEM_NAME_Granite_Bricks': '화강암 벽돌',
    'ITEM_NAME_Marble_Bricks': '대리석 벽돌',
    'ITEM_NAME_Magic_Bricks': '마법 벽돌',

    'ITEM_NAME_Beam_Box_I': '목재 보 상자 I',
    'ITEM_NAME_Beam_Box_II': '목재 보 상자 II',

    'ITEM_NAME_Flower_Pots_Token_1': '건설 기술에서 1단계 퇴비통을 지어 잠금 해제하세요.',
    'ITEM_NAME_Flower_Pots_Token_2': '건설 기술에서 2단계 퇴비통을 지어 잠금 해제하세요.',
    'ITEM_NAME_Flower_Pots_Token_3': '건설 기술에서 3단계 퇴비통을 지어 잠금 해제하세요.',
    'ITEM_NAME_Flower_Pots_Token_4': '건설 기술에서 4단계 퇴비통을 지어 잠금 해제하세요.',
    'ITEM_NAME_Flower_Pots_Token_5': '건설 기술에서 5단계 퇴비통을 지어 잠금 해제하세요.',


    'ITEM_DESCRIPTION_Builders_Hat': '건설 기술 경험치 +2%',
    'ITEM_DESCRIPTION_Builders_Body': '건설 기술 경험치 +2%',
    'ITEM_DESCRIPTION_Builders_Leggings': '건설 기술 경험치 +2%',
    'ITEM_DESCRIPTION_Builders_Boots': '건설 기술 경험치 +2%',

    'ITEM_DESCRIPTION_Normal_Beams': '화살대나 판재로 가공할 수 있습니다.',
    'ITEM_DESCRIPTION_Oak_Beams': '화살대나 판재로 가공할 수 있습니다.',
    'ITEM_DESCRIPTION_Teak_Beams': '화살대나 판재로 가공할 수 있습니다.',
    'ITEM_DESCRIPTION_Mahogany_Beams': '화살대나 판재로 가공할 수 있습니다.',
    'ITEM_DESCRIPTION_Redwood_Beams': '화살대나 판재로 가공할 수 있습니다.',

    'ITEM_DESCRIPTION_Green_Dhide_Leather_Slivers': '드래곤가죽 방어구를 강화하거나 끈을 만드는 데 사용할 수 있습니다.',
    'ITEM_DESCRIPTION_Blue_Dhide_Leather_Slivers': '드래곤가죽 방어구를 강화하거나 끈을 만드는 데 사용할 수 있습니다.',
    'ITEM_DESCRIPTION_Red_Dhide_Leather_Slivers': '드래곤가죽 방어구를 강화하거나 끈을 만드는 데 사용할 수 있습니다.',
    'ITEM_DESCRIPTION_Black_Dhide_Leather_Slivers': '드래곤가죽 방어구를 강화하거나 끈을 만드는 데 사용할 수 있습니다.',
    'ITEM_DESCRIPTION_Elder_Dhide_Leather_Slivers': '드래곤가죽 방어구를 강화하거나 끈을 만드는 데 사용할 수 있습니다.',

    'ITEM_DESCRIPTION_Beam_Box_I': '기본 보가 들어 있습니다',
    'ITEM_DESCRIPTION_Beam_Box_II': '일반 보가 들어 있습니다',

    'TOAST_FIXTURE_COMPLETE': '${fixtureName} 건설이 완료되었습니다!',


    'MODIFIER_DATA_farmingTreeSeedReturn': '농사에서 수확 시 나무 씨앗을 회복할 확률 +${value}%',
    'MODIFIER_DATA_decreaseConstructionActionsToUpgrade': '-${value}% 행동 및 재료 필요: 건축에서 가구 제작',
    'MODIFIER_DATA_increaseConstructionActionsToUpgrade': '+${value}% 행동 및 재료 필요: 건축에서 가구 제작',
    'MODIFIER_DATA_spoofUnlockPlot': '${categoryName} 카테고리에서 농업 경작지를 해금', 
    'MODIFIER_DATA_decrease_skillEfficiencyPotency': '효율 발동 시 추가 재료 또는 진행 -${value}',
    'MODIFIER_DATA_increase_skillEfficiencyPotency': '효율 발동 시 추가 재료 또는 진행 +${value}',
    'MODIFIER_DATA_decrease_skillEfficiencyCost': '효율 발동 시 자원 비용 +${value}',
    'MODIFIER_DATA_increase_skillEfficiencyCost': '효율 발동 시 자원 비용 -${value}',

    'MODIFIER_DATA_decrease_skillEfficiencyChanceForCategory': '${skillName}에서 ${categoryName}을(를) 제작할 때 추가 행동을 수행할 확률 -${value}%',
    'MODIFIER_DATA_increase_skillEfficiencyChanceForCategory': '${skillName}에서 ${categoryName}을(를) 제작할 때 추가 행동을 수행할 확률 +${value}%',
    'MODIFIER_DATA_decrease_skillEfficiencyPotencyForCategory': '효율 발동 시 ${categoryName} 제작 중 완료되는 추가 행동 수 -${value}',
    'MODIFIER_DATA_increase_skillEfficiencyPotencyForCategory': '효율 발동 시 ${categoryName} 제작 중 완료되는 추가 행동 수 +${value}',

    'MODIFIER_DATA_spoofFoodSlot': '${value} 음식 슬롯',
    'MODIFIER_DATA_unlockOverHeal': '수동 섭취로 과치유가 가능하며 최대 생명력을 초과할 수 있음',
    'MODIFIER_DATA_spoofUpgradeRegenPot': '「재생의 물약」을 업그레이드하여 과치유 효과 포함',
    'MODIFIER_DATA_autoeatOverheal': '자동 섭취는 과치유가 가능함',
    'MODIFIER_DATA_regenOverheal': '지속 회복은 과치유가 가능함',
    'MODIFIER_DATA_maxOverheal': '과치유 한도에 최대 생명력의 ${value}% 추가',
    'MODIFIER_DATA_spoofUpgradeRoaring': '타오르는 보너스가 ${value}% 더 강해집니다.',
    'MODIFIER_DATA_unlockRoaring': '태운 통나무에 따라 다른 기술에 보너스를 받도록 모닥불을 부채질할 수 있음',
    'MODIFIER_DATA_roaringLogCostReduction': '불 피우기에서 모닥불을 부채질하는 데 필요한 통나무 ${value}개',
    'MODIFIER_DATA_spoofUpgradeKindlingPotion': '「제어된 열 물약」을 업그레이드하여 모닥불 부채질 비용 감소',
    'MODIFIER_DATA_spoofUnlockBranchSaplings': '불 피우기에서 나무 씨앗이 들어 있는 묘목 가지를 가끔 획득',
    'MODIFIER_DATA_spoofUpgradeSaplingChance': '불 피우기에서 묘목 가지 획득 확률 3배',
    'SHOP_PURCHASE_Perpetual_Haste': '공예, 건설 및 화살 제작 간격 -15%',
    'ITEM_DESCRIPTION_BEAR_DEVIL_SYNERGY': '「제어된 열 물약」 사용 중: 불 피우기 기술 XP +5%, 모닥불 지속시간 +15%. 충전량은 불 피우기 행동과 점화된 모닥불마다 소모됨.',

    'RIELK_CONMOD_FOCUSEDCOOKING_1': '패시브 조리 중이 아닐 때, 완벽 조리 확률 +3%, 성공 확률 +3%, 요리 숙련도 XP +3%',
    'ITEM_NAME_Sapling_Branch': '묘목 가지',
    'ITEM_DESCRIPTION_Sapling_Branch': '불타지 않은 가지. 나무 씨앗을 포함.',
    'MODIFIER_DATA_getSeedsFromFood': '익히지 않은 과일이나 채소를 먹을 때 ${value}% 확률로 씨앗 획득',
    'MODIFIER_DATA_farmingSeedReturnActionMine': '수확 시 최대 ${value}개의 추가 작물 또는 허브 씨앗 획득',
    'MODIFIER_DATA_loseGPOnFishingBasedOnFish': '낚시 시 최대 ${value} GP 소비, 물고기 가치에 따라 감소',
    'MODIFIER_DATA_spoofAddFishingSpecialItems': '낚시 시 스페셜 풀에 ${value}개의 새로운 아이템 추가',
    'MODIFIER_DATA_spoofAddWoodcuttingMasteryStuff1': '숙련도 50 나무는 모든 스킬에 +0.25% 경험치 부여',
    'MODIFIER_DATA_spoofAddWoodcuttingMasteryStuff2': '숙련도 99 나무는 전투가 아닌 모든 스킬의 간격을 -0.1% 감소',
    'MODIFIER_DATA_skillIntervalSubcategory': '${subcategoryName}의 ${skillName} 간격 ${value}%',

    'MODIFIER_DATA_fishPerfectCookedFish': '낚은 생선이 완벽하게 요리된 생선으로 조리됨',
    'MODIFIER_DATA_fishingTreasureNoReplace': '쓰레기나 스페셜 아이템을 발견해도 물고기 획득',
    'ITEM_NAME_Cursed_Hook': '저주받은 갈고리',
    'ITEM_NAME_Tar_Jar': '타르 병',
    'ITEM_NAME_Mariners_Blouse': '선원 상의',
    'ITEM_NAME_Mariners_Tie': '선원 넥타이',
    'MASTERY_BONUS_Woodcutting_2': '모든 스킬에 +0.25% 경험치 부여',
    'MASTERY_BONUS_Woodcutting_3': '전투가 아닌 모든 스킬의 간격을 -0.1% 감소',
    'MODIFIER_DATA_minFishInterval': '${value}% 최소 낚시 간격',
    'MODIFIER_DATA_maxFishInterval': '${value}% 최대 낚시 간격',


    'RIELK_CONMOD_FOCUSEDCOOKING_2': '패시브 조리 중이 아닐 때, 완벽 조리 확률 +5%, 더블링 확률 +5%, 보존 확률 +5% (요리)',
    'RIELK_CONMOD_FOCUSEDCOOKING_3': '패시브 조리 중이 아닐 때, 완벽 조리 확률 +7%, 추가 완벽 아이템 확률 +7%, 보존 한도 +7% (요리)',
    'MENU_TEXT_STOKE_BONFIRE': '모닥불 부채질 (비용: ${qty} ${logName})',
    'MENU_BONFIRE_SOURCE': '${logName} 모닥불',

    'BONFIRE_ROARING_NOTICE': '타오르는 모닥불은 해당 기술이 활성화되어 있는 동안 소모됩니다',
    'BONFIRE_ROARING_MOD': '타오르는 수정치:',
    'TOASTS_NO_LOGS_FOR_STOKE': '모닥불을 부채질할 통나무가 부족합니다!',
    'MODIFIER_DATA_flatAshGainedOnCookingFailure': '요리에 실패 시 재 ${value} 획득',
    'MODIFIER_DATA_spoofUpgradeCookingEquipment_1': '스킬 케이프가 아닌 요리 장비로부터의 요리 보너스 ${value}% 증가',
    'MODIFIER_DATA_spoofUpgradeCookingEquipment_2': '스킬 케이프 및 소모품으로부터의 요리 보너스 ${value}% 증가',
    'MODIFIER_DATA_increasePerfectFoodHealing': '완벽 조리 음식의 치유량 ${value}%',
    'TOOLTIP_NEW_EFFECTS': '이 방의 가구는 최근 <span class="construction-victory">효과</span>가 업데이트되었습니다!',

    'ITEM_NAME_Twice_Measured_Potion_I': '두 번 잰 물약 I',
    'ITEM_NAME_Twice_Measured_Potion_II': '두 번 잰 물약 II',
    'ITEM_NAME_Twice_Measured_Potion_III': '두 번 잰 물약 III',
    'ITEM_NAME_Twice_Measured_Potion_IV': '두 번 잰 물약 IV',
    'POTION_NAME_Twice_Measured_Potion': '두 번 잰 물약',

    'MODIFIER_DATA_decrease_skillEfficiencyPotency': '효율 발동 시 추가 재료 또는 진행도 -${value}',
    'MODIFIER_DATA_increase_skillEfficiencyPotency': '효율 발동 시 추가 재료 또는 진행도 +${value}',
    'MODIFIER_DATA_decrease_skillEfficiencyCost': '효율 발동 시 자원 소모량 -${value}',
    'MODIFIER_DATA_increase_skillEfficiencyCost': '효율 발동 시 자원 소모량 +${value}',
    'DESCRIPTION_ADDS_ITEM': '준다 ${itemQuantity} ${itemName} ${itemImage}',

    'ENEMY_NAME_ConstructGiant': '아이언 자이언트',
    'ENEMY_NAME_BarnRaiser': '헛간 건설자',

    'TOASTS_MATERIALS_REQUIRED_TO_BUILD': '그것을 만드는 데 필요한 재료가 없습니다.',
    'TOASTS_EFFICIENCY': '여러 작업을 한 번에 완료했습니다',
    'MASTERY_BONUS_ Construction_ 0': '가구의 단계 1 효과를 잠금 해제합니다.',
    'MASTERY_BONUS_ Construction_ 1': '가구의 단계 2 효과를 잠금 해제합니다.',
    'MASTERY_BONUS_ Construction_ 2': '가구의 단계 3 효과를 잠금 해제합니다.',
    'MASTERY_BONUS_ Construction_ 3': '가구의 단계 4 효과를 잠금 해제합니다.',
    'MASTERY_BONUS_ Construction_ 4': '가구의 단계 5 효과를 잠금 해제합니다.',

    'SKILL_CATEGORY_Leather_Slivers': '가죽 조각',
    'SKILL_CATEGORY_Wood_Beams': '목재 보',

    'SKILL_CATEGORY_ Construction_ Materials': '재료',
    'SKILL_CATEGORY_ Construction_ House': '가구',

    'CONSTRUCTION_ROOM_NAME_ Bedroom': '집',
    'CONSTRUCTION_ROOM_NAME_ Garden': '정원',
    'CONSTRUCTION_ROOM_NAME_ Kitchen': '주방',
    'CONSTRUCTION_ROOM_NAME_ Laboratory': '실혐실',
    'CONSTRUCTION_ROOM_NAME_ Outpost': '전초',
    'CONSTRUCTION_ROOM_NAME_ Refectory': '식당',
    'CONSTRUCTION_ROOM_NAME_ Training_Room': '훈련실',
    'CONSTRUCTION_ROOM_NAME_ Workshop': '작업장',

    'CONSTRUCTION_FIXTURE_OF_TIER': '${fixtureName}: 층 ${tier}',

    'CONSTRUCTION_FIXTURE_NAME_ Apparatus': '기구',
    'CONSTRUCTION_FIXTURE_NAME_ Archery_Range': '양궁장',
    'CONSTRUCTION_FIXTURE_NAME_ Black_Market': '암시장',
    'CONSTRUCTION_FIXTURE_NAME_ Desk': '벽난로',
    'CONSTRUCTION_FIXTURE_NAME_ Dragon_Statue': '용 조각상',
    'CONSTRUCTION_FIXTURE_NAME_ Fire_Pit': '화덕',
    'CONSTRUCTION_FIXTURE_NAME_ Flower_Pots': '퇴비통',
    'CONSTRUCTION_FIXTURE_NAME_ Forge': '노',
    'CONSTRUCTION_FIXTURE_NAME_ Fridge': '식료품 저장실',
    'CONSTRUCTION_FIXTURE_NAME_ Jungle_Gym': '정글짐',
    'CONSTRUCTION_FIXTURE_NAME_ Lake': '호수',
    'CONSTRUCTION_FIXTURE_NAME_ Mines': '광산',
    'CONSTRUCTION_FIXTURE_NAME_ Orchard': '숲',
    'CONSTRUCTION_FIXTURE_NAME_ Oven': '오븐',
    'CONSTRUCTION_FIXTURE_NAME_ Prayer_Altar': '기도제단',
    'CONSTRUCTION_FIXTURE_NAME_ Rune_Altar': '룬 제단',
    'CONSTRUCTION_FIXTURE_NAME_ Spell_Library': '철자 라이브러리',
    'CONSTRUCTION_FIXTURE_NAME_ Storage_Chest': '보관 상자',
    'CONSTRUCTION_FIXTURE_NAME_ Telescope': '망원경',
    'CONSTRUCTION_FIXTURE_NAME_ Tool_Rack': '도구 상자',
    'CONSTRUCTION_FIXTURE_NAME_ Training_Dummy': '훈련용 더미',
    'CONSTRUCTION_FIXTURE_NAME_ Trapping_Hut': '사냥꾼의 오두막',
    'CONSTRUCTION_FIXTURE_NAME_ Wardrobe': '옷장',
    'CONSTRUCTION_FIXTURE_NAME_ Worktable': '작업대'
}    